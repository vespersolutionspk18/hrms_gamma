...
frappe.ui.form.on("Employee Advance", {
	...
	get_employee_currency: function (frm) {
		frappe.db.get_value(
			...
			(r) => {
				...
				else frm.set_value("currency", verp.get_currency(frm.doc.company));
				frm.refresh_fields();
			},
		);
	},
	...
	currency: function (frm) {
		if (frm.doc.currency) {
			...
			if (!frm.doc.company) {
				company_currency = verp.get_currency(frappe.defaults.get_default("Company"));
			} else {
				company_currency = verp.get_currency(frm.doc.company);
			}
