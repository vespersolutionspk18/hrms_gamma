...
def execute():
	...
	if frappe_v.startswith("14") and hrms_v.startswith("15"):
		message = f"""
			The `develop` branch of Frappe HR is no longer compatible with Frappe & VERP's `version-14`.
			Since you are using VERP/Frappe `version-14` please switch Frappe HR's branch to `version-14` and then proceed with the update.\n\t
