...
class TestSalarySlip(FrappeTestCase):
	...
	def test_payroll_frequency(self):
		fiscal_year = get_fiscal_year(nowdate(), company=verp.get_default_company())[0]
...
def make_leave_application(
	...
	leave_application = frappe.get_doc(
		dict(
			...
			company=company or verp.get_default_company() or "_Test Company",
			status="Approved",
...
def setup_test():
	...
	frappe.db.set_value(
		"Company", verp.get_default_company(), "default_holiday_list", "Salary Slip Test Holiday List"
	)
...
def make_payroll_period():
	default_company = verp.get_default_company()
...
def make_holiday_list(list_name=None, from_date=None, to_date=None, add_weekly_offs=True):
	fiscal_year = get_fiscal_year(nowdate(), company=verp.get_default_company())
