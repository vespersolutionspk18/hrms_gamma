...
def subscription_updated(app: str, plan: str):
	if app in ["hrms", "verp"] and plan:
		update_verp_access()


def update_verp_access():
	"""
	ignores if user has no hrms subscription
	enables verp workspaces and roles if user has subscribed to hrms and verp
	disables verp workspaces and roles if user has subscribed to hrms but not verp
	"""
	if not frappe.utils.get_url().endswith(".verp.io"):
		return

	update_verp_workspaces(True)
	update_verp_roles(True)
	set_app_logo()


def update_verp_workspaces(disable: bool = True):
	verp_workspaces = [
		"Home",
		"Assets",
		"Accounting",
	...
	for workspace in verp_workspaces:
		try:
...
def update_verp_roles(disable: bool = True):
	roles = get_verp_roles()
	for role in roles:
		try:
...
def get_verp_roles() -> set:
	verp_roles = get_roles_for_app("verp")
	hrms_roles = get_roles_for_app("hrms")
	return verp_roles - hrms_roles - set(STANDARD_ROLES)


def get_roles_for_app(app_name: str) -> set:
	verp_modules = get_modules_by_app(app_name)
	doctypes = get_doctypes_by_modules(verp_modules)
...
def hide_verp() -> bool:
	...
	verp_subscription = has_subscription(frappe.conf.sk_verp_smb or frappe.conf.sk_verp)

	if not hr_subscription:
		return False

	if hr_subscription and verp_subscription:
		# subscribed for VERP
		return False

	# no subscription for VERP
	return True


def has_subscription(secret_key) -> bool:
	url = f"https://verp.io/api/method/press.api.developer.marketplace.get_subscription_status?secret_key={secret_key}"
